...
import { isAbortError, type MetricsResult } from '../contracts';
import { AutoSuggestController } from './AutoSuggestController.svelte';
import {
...
function fakePipeline(ssimAt: (quality: number) => number | null) {
  const tried: number[] = [];
  const encode = async (quality: number): Promise<AutoSuggestEncodeResult> => {
    tried.push(quality);
    return { size: quality * 1000, data: solid(8, 8, quality) };
  };
  const measure = async (_output: MetricsImage, quality: number) => ssimAt(quality);
  return { tried, encode, measure };
}

/** SSIM rises linearly with quality: q90 → 0.99, the search's default target. */
const linearSsim = (quality: number) => 0.9 + quality / 1000;

describe('suggestQuality', () => {
  it('finds the smallest quality that reaches the target', async () => {
    const { tried, encode, measure } = fakePipeline(linearSsim);
    const suggestion = await suggestQuality(encode, measure);

    expect(suggestion.quality).toBe(90);
    expect(suggestion.met).toBe(true);
    expect(suggestion.ssim).toBeCloseTo(0.99, 6);
    expect(suggestion.size).toBe(90_000);
    expect(suggestion.iterations).toBe(AUTO_SUGGEST_DEFAULTS.maxIters);
    expect(tried.length).toBe(suggestion.iterations);
    expect(new Set(tried).size).toBe(tried.length); // never probes the same q twice
  });

  it('honours a custom range and target', async () => {
    const { encode, measure } = fakePipeline(linearSsim);
    const suggestion = await suggestQuality(encode, measure, {
      targetSsim: 0.95,
      qMin: 10,
      qMax: 80,
    ...
    expect(suggestion.quality).toBe(50);
    expect(suggestion.met).toBe(true);
  });

  it('reports the closest attempt when nothing reaches the target', async () => {
    const { tried, encode, measure } = fakePipeline(() => 0.5);
    const suggestion = await suggestQuality(encode, measure);

    expect(suggestion.met).toBe(false);
    expect(suggestion.ssim).toBe(0.5);
    expect(suggestion.quality).toBe(tried[tried.length - 1]);
  });

  it('degenerate: converges to qMin when every probe already clears the target', async () => {
    const { tried, encode, measure } = fakePipeline(() => 0.999);
    const suggestion = await suggestQuality(encode, measure);

    expect(suggestion.met).toBe(true);
    expect(suggestion.quality).toBe(AUTO_SUGGEST_DEFAULTS.qMin);
    expect(tried.length).toBeLessThanOrEqual(AUTO_SUGGEST_DEFAULTS.maxIters);
  });

  it('treats an unmeasurable encode as a failure to reach the target', async () => {
    const { encode, measure } = fakePipeline(() => null);
    const suggestion = await suggestQuality(encode, measure, { maxIters: 2 });
    expect(suggestion.met).toBe(false);
    expect(suggestion.ssim).toBeNull();
  });

  it('accepts a MetricsResult as well as a bare number', async () => {
    const encode = async (quality: number): Promise<AutoSuggestEncodeResult> => ({
      size: quality,
      data: solid(4, 4, 1),
    });
    const measure = async (
      _output: MetricsImage,
      quality: number,
    ): Promise<MetricsResult> => ({ ssim: linearSsim(quality), ms: 1 });

    const suggestion = await suggestQuality(encode, measure);
    expect(suggestion.quality).toBe(90);
  });

  it('never runs more than maxIters rounds', async () => {
    const { tried, encode, measure } = fakePipeline(linearSsim);
    const suggestion = await suggestQuality(encode, measure, { maxIters: 2 });
    expect(tried.length).toBe(2);
    expect(suggestion.iterations).toBe(2);
  });
  ...
  it('rejects an already-aborted signal', async () => {
    ...
    await expectAbortError(suggestQuality(encode, measure, { signal: controller.signal }));
  });

  it('stops mid-search when the signal fires', async () => {
    const controller = new AbortController();
    const tried: number[] = [];
    const encode = async (quality: number): Promise<AutoSuggestEncodeResult> => {
      tried.push(quality);
      controller.abort();
      return { size: 1, data: solid(4, 4, 1) };
    };
    const measure = async () => 0.5;
  ...
  it('rejects an inverted range', async () => {
    const { encode, measure } = fakePipeline(linearSsim);
    await expect(suggestQuality(encode, measure, { qMin: 90, qMax: 10 })).rejects.toThrow(
      RangeError,
    );
  });

  it('normalises measurement shapes', () => {
    expect(ssimOf(0.9)).toBe(0.9);
    expect(ssimOf({ ssim: 0.8, ms: 2 })).toBe(0.8);
    expect(ssimOf({ ssim: null, ms: 2 })).toBeNull();
    expect(ssimOf(null)).toBeNull();
    expect(ssimOf(Number.NaN)).toBeNull();
  });
});

describe('describeSuggestion', () => {
  it('writes the comp sentence', () => {
    const message = describeSuggestion(
      {
        quality: 52,
        size: 390_000,
        ssim: 0.994,
      ...
      { encoderLabel: 'AVIF', originalBytes: 3_000_000 },
    );
    expect(message).toBe('AVIF at q52 is visually identical — 13% of the original weight');
  });

  it('falls back to an absolute size without the original', () => {
    const message = describeSuggestion(
      {
        quality: 52,
        size: 390_000,
    ...
    expect(message).toBe('AVIF at q52 is visually identical — 390 kB');
  });
  ...
  it('says so when the target was missed', () => {
    const message = describeSuggestion(
      ...
      { encoderLabel: 'MozJPEG', originalBytes: 1_000_000 },
    );
    expect(message).toContain('as close as it gets — SSIM 0.960');
    expect(message).toContain('90% of the original weight');
  });

  it('formats bytes and shares', () => {
    expect(formatBytes(948)).toBe('948 B');
    expect(formatBytes(390_000)).toBe('390 kB');
    expect(formatBytes(2_920_000)).toBe('2.92 MB');
    expect(percentOfOriginal(3_000_000, 390_000)).toBe(13);
    expect(percentOfOriginal(0, 100)).toBe(0);
  });
});
...
describe('AutoSuggestController', () => {
  const controllerFor = (
    overrides: Partial<ConstructorParameters<typeof AutoSuggestController>[0]> = {},
  ) => {
    const { encode, measure } = fakePipeline(linearSsim);
  ...
  it('starts idle and silent', () => {
    const auto = controllerFor();
    expect(auto.state).toBe('idle');
    expect(auto.message).toBeNull();
    expect(auto.visible).toBe(false);
  });

  it('runs, lands on done, and writes the pill sentence', async () => {
    const auto = controllerFor();
    const suggestion = await auto.run();

    expect(suggestion?.quality).toBe(90);
    expect(auto.state).toBe('done');
    expect(auto.suggestion?.quality).toBe(90);
    expect(auto.message).toBe(
      'AVIF at q90 is visually identical — 9% of the original weight',
    );
    expect(auto.actionLabel).toBe('Apply');
    expect(auto.visible).toBe(true);
  });

  it('hands the suggestion to onApply exactly once per run', async () => {
    const onApply = vi.fn();
    const auto = controllerFor({ onApply });
    await auto.run();

    auto.apply();
    expect(onApply).toHaveBeenCalledTimes(1);
    expect(onApply.mock.calls[0]?.[0]).toMatchObject({ quality: 90 });
    expect(auto.applied).toBe(true);
    expect(auto.actionLabel).toBe('Applied');
  });

  it('reports a failing pipeline instead of throwing', async () => {
    const auto = controllerFor({
      encodeAt: () => Promise.reject(new Error('encoder exploded')),
    });

    await expect(auto.run()).resolves.toBeNull();
    expect(auto.state).toBe('error');
    expect(auto.message).toBe('encoder exploded');
    expect(auto.actionLabel).toBe('Retry');
  });
  ...
  it('goes back to idle when cancelled', async () => {
    ...
    const auto = controllerFor({
      encodeAt: async (quality) => {
        await gate;
        return { size: quality, data: solid(4, 4, 1) };
      },
    });

    const running = auto.run();
    expect(auto.state).toBe('running');
    expect(auto.message).toMatch(/Testing q|Looking for/);
  ...
  it('supersedes an earlier run', async () => {
    const auto = controllerFor();
    const first = auto.run();
    const second = auto.run();

    await expect(first).resolves.toBeNull();
    expect((await second)?.quality).toBe(90);
    expect(auto.state).toBe('done');
  });
  ...
  it('resets', async () => {
    ...
    expect(auto.state).toBe('idle');
    expect(auto.suggestion).toBeNull();
