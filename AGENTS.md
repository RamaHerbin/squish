...
`public/demo/demo-report.pdf` is generated by the same script (`pdf` target) but
by `@cantoo/pdf-lib`, not Chrome: it re-embeds the two demo rasters into a
two-page report, with its dates pinned so the bytes are stable.
